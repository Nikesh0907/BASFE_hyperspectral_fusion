...
def _conv_prelu(x, filters, k):
    x = layers.Conv2D(filters, k, activation=None, padding="same", use_bias=True)(x)
    x = layers.PReLU()(x)
    return x


def _spec(inputs, nf):
    x = _conv_prelu(inputs, nf, 1)
    x = _conv_prelu(x, nf, 1)
    return layers.Add()([inputs, x])


def _spat(inputs, nf):
    x = _conv_prelu(inputs, nf, 3)
    x = _conv_prelu(x, nf, 3)
    return layers.Add()([inputs, x])
